...
const SignUp = () => {
    ...
    const postDetails = (pics) => {
        ...
        if(pics.type==="image/jpeg"|| pics.type==="image/png"){
            ...
            data.append("cloud_name","surajsehgal");
            fetch("https://api.cloudinary.com/v1_1/surajsehgal/image/upload",{
                method: "post",
                body: data
            })
            .then((res)=> res.json())
    ...
    return (
        <VStack spacing="5px">
            ...
            <FormControl id='cpassword' isRequired>
                ...
                <InputGroup>
                    <Input type={show ? 'text' : 'password'} placeholder='Confirm Password' onChange={(e) => setCPassword(e.target.value)} />
                    <InputRightElement>
                        <Button onClick={handleClick} h={"1.75rem"} size={"sm"} margin="0 10px 0 0" px={"5"}>{show ? "Hide" : "Show"}</Button>
                    </InputRightElement>
                </InputGroup>
            ...
            <Button  width={"100%"} colorScheme="blue" style={{ marginTop: 15 }} onClick={submitHandler} isLoading={loading}>Sign Up</Button>
        </VStack>
    )
}
